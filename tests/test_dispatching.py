...
class TestDispatching(unittest.TestCase):
    ...
    def test_simple_save_emit(self):
        ...
        def test(res, val):
            self.assertEqual(self.target.called, val)
    ...
    def test_emit_without_receivers(self):
        self.dispatcher.emit('test')
        self.assertEqual(self.target.called, 0)
    ...
    def test_emit_with_multiple_receiver(self):
        ...
        # check rc3
        self.assertEqual(self.target.one, 'other')
        self.assertEqual(self.target.one_other, 'q')
        ...
        # check rc3
        self.assertEqual(self.target.one, 'other')
        self.assertEqual(self.target.one_other, 'thistime')

        # no keyword
        self.dispatcher.emit('test', self, 'a')
        # worked for rc1 and rc3 with the default value
        self.assertEqual(self.target.va1_a, 'p')
        self.assertEqual(self.target.one_a, 'p')
        # but not on rc2
    ...
    def test_emit_multiple_with_failing_in_between(self):
        ...
        self.assertEqual(self.target.called_b, 2)
        self.assertEqual(self.target.called_c, 0)
        self.assertEqual(self.target.called_d, 4)

        self.dispatcher.emit('test')
        self.assertEqual(self.target.called_a, 2)
        self.assertEqual(self.target.called_b, 4)
        self.assertEqual(self.target.called_c, 0)
        self.assertEqual(self.target.called_d, 8)
...
class TestReceiver(unittest.TestCase):
    ...
    def test_calling_with_args(self):
        ...
        rec(0)
        self.assertEqual(self.called, 1)
        self.assertEqual(self.args, (0, 1, 2, 3))
        self.assertEqual(self.kw, {'test': 'a'})

        rec(-1)
        self.assertEqual(self.called, 2)
        self.assertEqual(self.args, (-1, 1, 2, 3))
        self.assertEqual(self.kw, {'test': 'a'})

        rec(-2)
        self.assertEqual(self.called, 3)
        self.assertEqual(self.args, (-2, 1, 2, 3))
        self.assertEqual(self.kw, {'test': 'a'})
    ...
    def test_calling_with_kw(self):
        ...
        rec(p='q')
        self.assertEqual(self.called, 1)
        self.assertEqual(self.args, (1, 2, 3))
        self.assertEqual(self.kw, {'test': 'a', 'p': 'q'})

        rec(other='wise')
        self.assertEqual(self.called, 2)
        self.assertEqual(self.args, (1, 2, 3))
        self.assertEqual(self.kw, {'test': 'a', 'other': 'wise'})
    ...
    def test_calling_with_clashing_kw_and_args(self):
        ...
        self.assertEqual(self.called, 1)
        self.assertEqual(self.args, (1, 2, 3))
        self.assertEqual(self.kw, {'test': 'a', 'p': 'a'})

        rec(1, 2, 7, test='True', o='p')
        self.assertEqual(self.called, 2)
        self.assertEqual(self.args, (1, 2, 7, 1, 2, 3))
        self.assertEqual(self.kw, {'test': 'True', 'o': 'p', 'p': 'a'})
...
class TestSignalingDescriptors(unittest.TestCase):
    ...
    def test_custom(self):
        ...
        self.signaler.x = 'Pocahontas'
        self.assertEqual(self.signaler.emitted, [])
        self.assertEqual(self.signaler.x, 'Pocahontas')
    ...
    def _check(self, values=[], x=0, x_set=0, x_get=0):
        self.assertEqual(self.signaler._x, x)
        self.assertEqual(self.signaler.x_set, x_set)
        self.assertEqual(self.signaler.x_get, x_get)
        self.assertEqual(self.signaler.emitted, values)
...
class TestStayInObjectSignaling(unittest.TestCase):
    ...
    def test_double_different_values(self):
        self.foo.simple = 'A'
        self.bar.simple = 'B'
        self.assertEqual(self.foo.simple, 'A')
        self.assertEqual(self.bar.simple, 'B')
        self.assertEqual(len(self.foo.emitted), 1)
        self.assertEqual(len(self.bar.emitted), 1)

        self.assertEqual(self.foo.emitted[0][1][0], 'A')
        self.assertEqual(self.bar.emitted[0][1][0], 'B')
