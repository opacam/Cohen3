...
def require(packageName, fixName):
    ...
    if (packageName, fixName) == ('twisted', 'filepath_copyTo'):
        ...
        if filepath.FilePath('a') != filepath.FilePath('a'):
            from coherence.extern.twisted.epsilon.hotfixes import filepath_copyTo
        ...
        if not hasattr(policies.TimeoutMixin, 'callLater'):
            from coherence.extern.twisted.epsilon.hotfixes import timeoutmixin_calllater
        ...
        if 'seconds' not in args:
            from coherence.extern.twisted.epsilon.hotfixes import delayedcall_seconds
        ...
        if result == [1]:
            from coherence.extern.twisted.epsilon.hotfixes import deferredgenerator_tfailure
        ...
        try:
            ...
            from coherence.extern.twisted.epsilon.hotfixes import proto_helpers_stringtransport
            proto_helpers_stringtransport.install()
    elif (packageName, fixName) == ("twisted", "internet_task_Clock"):
        from twisted.internet.task import Clock
        from twisted.internet import base
        from twisted import version
        from coherence.extern.twisted.epsilon.hotfixes import internet_task_clock
        ...
        if not hasattr(TestCase, "failUnlessWarns"):
            from coherence.extern.twisted.epsilon.hotfixes import trial_assertwarns
            trial_assertwarns.install()
    elif (packageName, fixName) == ("twisted", "plugin_package_paths"):
        try:
            from twisted.plugin import pluginPackagePaths
        except ImportError:
            from coherence.extern.twisted.epsilon.hotfixes import plugin_package_paths
        ...
        if (version.major, version.minor) < (8, 2):
            from coherence.extern.twisted.epsilon.hotfixes import loopbackasync_reentrancy
