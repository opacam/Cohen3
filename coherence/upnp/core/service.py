...
class Service(EventDispatcher, log.LogAble):
    ...
    def __init__(
        ...
        self.control_url = (
            ...
            else control_url.encode('ascii')
            if control_url
            else None
        )
        self.event_sub_url = (
            event_sub_url
            if isinstance(event_sub_url, bytes)
            else event_sub_url.encode('ascii')
            if event_sub_url
            else None
        )
        self.presentation_url = (
            presentation_url
            if isinstance(presentation_url, bytes)
            else presentation_url.encode('ascii')
            if presentation_url
            else None
        )
        self.scpd_url = (
            scpd_url
            if isinstance(scpd_url, bytes)
            else scpd_url.encode('ascii')
            if scpd_url
            else None
        )
        self.device = device
        ...
        self.parse_actions()
        self.info(
            f'{device.friendly_name} {self.service_type} '
    ...
    def remove(self):
        self.info(
            f'removal of  {self.device.friendly_name} '
    ...
    def set_timeout(self, timeout):
        self.info(
            f'set timout for {self.device.friendly_name}/'
        ...
        try:
            ...
            self.info(
                f'reset renew subscription call for '
            ...
            self.info(
                f'starting renew subscription call for '
                + f'{self.device.friendly_name}/{self.service_type} to '
                + f'{int(self.timeout) - 30:d}'
            )
    ...
    def set_sid(self, sid):
        self.info(
            f'set subscription id for {self.device.friendly_name}/'
    ...
    def subscribe_for_variable(
        self, var_name, instance=0, callback=None, signal=False
    ):
        state_variable = self.get_state_variable(var_name)
        if state_variable:
            if callback is not None:
                if signal:
                    callback(state_variable)
                    state_variable.bind(state_variable_changed=callback)
                else:
                    state_variable.subscribe(callback)
    ...
    def process_event(self, event):
            ...
            if var_name == 'LastChange':
                ...
                for instance in tree.findall(f'{{{namespace_uri}}}InstanceID'):
                    ...
                    for var in instance.getchildren():
                        ...
                        if len(var.attrib) > 1:
                            self.info(
                                f'Extended StateVariable '
                                + f'{var.tag} - {var.attrib}'
                            )
                            if (
                                'channel' in var.attrib
                                and var.attrib['channel'] != 'Master'
                            ):
        ...
        if self.last_time_updated is None:
            ...
            self.info(
                f'send signal Coherence.UPnP.DeviceClient.Service.notified for'
                + f' {self}'
            )
        self.last_time_updated = time.time()
    ...
    def parse_actions(self):
        ...
        def gotPage(x):
            ...
            try:
                ...
                self.warning(
                    f'Invalid service description received from '
            ...
            for action_node in tree.findall(f'.//{{{ns}}}action'):
                ...
                self._actions[name] = action.Action(
                    self, name, 'n/a', arguments
                )
            ...
            for var_node in tree.findall(f'.//{{{ns}}}stateVariable'):
                ...
                self._variables.get(instance)[name] = variable.StateVariable(
                    self, name, 'n/a', instance, send_events, data_type, values
                )
...
class ServiceServer(log.LogAble):
    ...
    def __init__(self, id, version, backend):
        ...
        try:
            ...
            pass
        self.debug(
            f'ServiceServer.__init__: putChild '
    ...
    def new_subscriber(self, subscriber):
        ...
        root = etree.Element(
            f'{{{NS_UPNP_ORG_EVENT_1_0}}}propertyset',
    ...
    def create_new_instance(self, instance):
        ...
        for v in list(self._variables[0].values()):
            ...
            self._variables[instance][
                v.name
            ].has_vendor_values = v.has_vendor_values
            ...
            self._variables[instance][
                v.name
            ].dependant_variable = v.dependant_variable
    ...
    def set_variable(self, instance, variable_name, value, default=False):
        def process_value(result):
            ...
            if (
                ...
                and not variable.moderated
                and len(self._subscribers) > 0
            ):
                xml = self.build_single_notification(
                    instance, variable_name, variable.value
                )
        ...
        for instance, vdict in list(self._variables.items()):
            ...
            for variable in list(vdict.values()):
                if (
                    ...
                    and not variable.never_evented
                ):
                    if variable.updated or force:
                        ...
                        if variable.dependant_variable is not None:
                            dependants = (
                                variable.dependant_variable.get_allowed_values()
                            )
    ...
    def register_vendor_variable(
        ...
        if evented == 'never':
            ...
            send_events = evented
        new_variable = variable.StateVariable(
            self,
            name,
            implementation,
            instance,
        ...
        if default_value is None:
            ...
            new_variable.default_value = (
                new_variable.old_value
            ) = new_variable.value = default_value
    ...
    def register_vendor_action(
        self, name, implementation, arguments=None, needs_callback=True
    ):
        '''
    ...
    def init_var_and_actions(self):
        ...
        for action_node in tree.findall('.//action'):
            ...
            if action_node.find('Optional') is not None:
                implementation = 'optional'
                if (
                    action_node.find('Optional').attrib.get(
                        '{urn:schemas-beebits-net:service-1-0}X_needs_backend',
                        None,
                    )
                    is not None
            ...
            for argument in action_node.findall('.//argument'):
                ...
                if (
                    arg_state_var[0:11] == 'A_ARG_TYPE_'
        ...
        backend_vendor_value_defaults = getattr(
            self.backend, 'vendor_value_defaults', None
        )
        ...
        backend_vendor_range_defaults = getattr(
            self.backend, 'vendor_range_defaults', None
        )
        ...
        for var_node in tree.findall('.//stateVariable'):
            ...
            if allowed_value_list is not None:
                ...
                if service_value_defaults:
                    ...
                    if variable_value_defaults:
                        self.info(
                            f'overwriting {name} default value '
            ...
            if allowed_value_range:
                ...
                for e in list(allowed_value_range):
                    ...
                    if vendor_values is not None:
                        if service_range_defaults:
                            var_range_defaults = service_range_defaults.get(
                                name
                            )
                            if (
                                var_range_defaults is not None
                                and var_range_defaults.get(e.tag)
                                is not None
                            ):
                                self.info(
                                    f'overwriting {name} attribute {e.tag} '
                                    + f'with {str(var_range_defaults[e.tag])}'
                                )
                                range[e.tag] = var_range_defaults[e.tag]
                            elif e.text is None:
                                self.info(
                                    f'missing vendor definition for '
                ...
                var_range_defaults = service_range_defaults.get(name)
                if var_range_defaults is not None:
                    self._variables.get(instance)[
                        name
                    ].set_allowed_value_range(**var_range_defaults)
                    self._variables.get(instance)[
                        name
                    ].has_vendor_values = True
...
class scpdXML(static.Data, log.LogAble):
    ...
    def build_xml(self):
        ...
        root = etree.Element(
            'scpd', nsmap={None: 'urn:schemas-upnp-org:service-1-0'}
        )
        ...
        for var in list(self.service_server._variables[0].values()):
            ...
            if (
                var.allowed_value_range is not None
...
class ServiceControl(log.LogAble):
    def get_action_results(self, result, action, instance):
        ...
        for argument in action.get_out_arguments():
            ...
            if argument.name[0:11] != 'A_ARG_TYPE_':
                instance_var = self.variables[instance][  # pylint: disable=no-member  # noqa: E501
                    argument.get_state_variable()
                ]
                if action.get_callback() is not None:
                    instance_var.update(r[argument.name])
                    if (
                        instance_var.send_events == 'yes'
                        and not instance_var.moderated
                    ):
                        notify.append(instance_var)
                else:
                    r[argument.name] = instance_var.value
    ...
    def soap__generic(self, *args, **kwargs):
        ...
        '''
        try:
            action = self.actions[kwargs['soap_methodName']]  # pylint: disable=no-member  # noqa: E501
        except KeyError:
        ...
        if 'X_UPnPClient' in kwargs and kwargs['X_UPnPClient'] == 'XBox':
        ...
        if len(in_arguments) > 0:
            ...
            self.critical(
                f'argument {args_names} ' f'missing for action {action.name}'
            )
            return failure.Failure(errorCode(402))
